...
public function findReclamationsByUser($userId)
{
    return $this->createQueryBuilder('r')
        ->where('r.user = :userId')
...
{
    $qb = $this->createQueryBuilder('r'); // Create and store the QueryBuilder instance
    $qb->andWhere('r.user = :userId')      // Add condition to filter by user ID
        ->setParameter('userId', $userId); // Bind the 'userId' parameter

    // Get the query from the QueryBuilder and then execute it to get results
